...
if not main_act_path:
    ...
    java = (
        ...
        "\n"
        "        // Polling runnable — checks URL bar AND page HTML for the !6s token\n"
        ...
        "                }\n"
        "\n"
        "                // Check URL bar first, then scan page HTML for lh3 links\n"
        "                // The token may be percent-encoded as %2F etc in the URL\n"
        "                wv.evaluateJavascript(\n"
        "                    \"(function(){\" +\n"
        "                    \"  function extract(s){\" +\n"
        "                    \"    var m=s.match(/[!%]6s(https?[^!\\\"'\\\\s<>]+lh3\\\\.googleusercontent\\\\.com[^!\\\"'\\\\s<>]+)/);\" +\n"
        "                    \"    if(!m)return null;\" +\n"
        "                    \"    try{return decodeURIComponent(m[1].replace(/\\\\+/g,' '));}catch(e){return m[1];}\" +\n"
        "                    \"  }\" +\n"
        "                    \"  var r=extract(window.location.href);\" +\n"
        "                    \"  if(r)return r;\" +\n"
        "                    \"  r=extract(document.documentElement.innerHTML);\" +\n"
        "                    \"  if(r)return r;\" +\n"
        "                    \"  var links=document.querySelectorAll('a[href]');\" +\n"
        "                    \"  for(var i=0;i<links.length;i++){r=extract(links[i].href);if(r)return r;}\" +\n"
        "                    \"  return null;\" +\n"
        "                    \"})()\",\n"
        "                    value -> {\n"
        "                        if (value != null && !value.equals(\"null\")\n"
        "                                && !value.isEmpty() && value.contains(\"lh3\")) {\n"
        "                            found[0] = true;\n"
        "                            sheet.dismiss();\n"
        "                            // Strip surrounding JS string quotes added by evaluateJavascript\n"
        "                            String token = value;\n"
        ...
        "\n"
        "        wv.loadUrl(mapsUrl);\n"
        "        handler.postDelayed(pollRef[0], 2500); // wait 2.5s for Maps to start loading SV\n"
